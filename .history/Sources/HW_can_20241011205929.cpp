...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    if (HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &rx_header, can_rx_data) == HAL_OK) // 获得接收到的数据头和数据
    {
        if(rx_header.StdId == 0x100){//帧头校验
            //校验通过进行具体数据处理
            can_success_times++;
           CAN_rx_data.tick = (can_rx_data[0] << 24) | (can_rx_data[1] << 16) | (can_rx_data[2] << 8) | can_rx_data[3]; 
           int16_t temp1 = (can_rx_data[4] << 8) | can_rx_data[5];
           CAN_rx_data.value1 = temp1 / 30000.0f;
           int16_t temp2 = (can_rx_data[6] << 8) | can_rx_data[7];
           CAN_rx_data.value2 = temp2 / 30000.0f;
        }
