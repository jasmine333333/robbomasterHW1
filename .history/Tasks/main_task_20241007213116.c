 ...
 * @LastEditTime: 2024-10-07 21:31:16
...
uint8_t Tx_Buffer[9];
uint8_t Rx_Buffer[9];
sturct uart uart_tx_data,uart_rx_data;
...
void MainTask(void)
{
    ...
    Can_Tx_Buffer[2] = tick >> 8;
    Can_Tx_Buffer[3] = tick ;
    Can_Tx_Buffer[4] = uart_tx_data.value >> 24;
    Can_Tx_Buffer[5] = uart_tx_data.value >> 16;
    Can_Tx_Buffer[6] = uart_tx_data.value >> 8;
    Can_Tx_Buffer[7] = uart_tx_data.value ;
...
void decode(uint8_t *Rx_Buffer,struct uart u2)
{
    ...
    {
        uart_rx_data.tick = (Rx_Buffer[3]<<24) | (Rx_Buffer[4]<<16) | (Rx_Buffer[5]<<8) | (Rx_Buffer[6]<<0) ;
        uart_rx_data.value = (Rx_Buffer[7]<<8) | (Rx_Buffer[8]<<0) ;
        rx_state = 2;
    }
    else if (rx_state == 2)
    {
